...
def Translate(txt):
	try:
		url = 'http://218.193.131.250:11111/'
		postdat = {'p':txt}
		resp = requests.post(url, data=postdat)
		ret = resp.content.decode('utf-8', errors='replace')
	except Exception as e:
		print(e)
		ret = ''
	return ret
...
def SortRows(file, fobj, cid, type = int, rev = True):
	with open(file, encoding = "utf-8") as filein:
		lines = [ll for ll in filein.read().split('\n') if ll != ""]
	dat = []
	for line in lines:
		dv = line.split('\t')
		dat.append((type(dv[cid]), line))
	with open(fobj, "w", encoding = "utf-8") as fout:
