...
def getTrainingData():
    global left_X, left_Y, right_X, right_Y, movie_X_train, movie_Y_train, movie_X_test, movie_Y_test, movie_X_dev, movie_Y_dev,  tweetX_train, tweetY_train, tweet_X_test, tweet_Y_test, w2v
    ...
    movie_X_train = getX(data_path  + 'movie_X_train.h5')
    movie_Y_train = getY(data_path  + 'movie_Y_train.h5')
    movie_X_test = getX(data_path  + 'movie_X_test.h5')
    movie_Y_test = getY(data_path  + 'movie_Y_test.h5')
    movie_X_dev = getX(data_path  + 'movie_X_dev.h5')
    movie_Y_dev = getY(data_path  + 'movie_Y_dev.h5')
    tweetX_train = getX(data_path  + 'tweet_X_train.h5')
    tweetY_train = getY(data_path  + 'tweet_Y_train.h5')
    tweet_X_test = getX(data_path  + 'tweet_X_test.h5')
    tweet_Y_test = getY(data_path  + 'tweet_Y_test.h5')
    w2v = getX('w2v.h5')
    w2v = [w2v]
...
def build_model(vocabulary_size, vector_dim, seqlen, w2v):
    model = Sequential()
    ...
    model.add(Embedding(input_dim = vocabulary_size, output_dim = vector_dim, input_length = seqlen, mask_zero = True, weights = w2v, name = 'embed'))
...
testX = left_X[: spix]
testY = left_Y[: spix]
model = build_model(vocabulary_size, vector_dim, seqlen, w2v)
print(len(left_X))
...
for k in range(0, 7):
    print(k)
    model.fit(tweet_X_train, tweet_Y_train, batch_size = 1024, nb_epoch = 1, verbose = 0, validation_data = (tweet_X_dev, tweet_Y_dev))
    print('tweet')
    showAcc(model, tweet_X_test, tweet_Y_test)
    print('movie')
    showAcc(model, movie_X_test, movie_Y_test)
    model.save_weights('lstm_transfer.h5', overwrite = True)
...
for i in range(0, 20):
    ...
    model.fit(trainxx, trainyy, batch_size = 32, nb_epoch = 1, verbose = 1, validation_data = (devxx, devyy))
